...
TCL_FILE_NAME = '/home/changhan/Documents/leo_compact_routing/BachelorProject/spacex_constellation/spacex_phase1_550km.tcl'
ALTITUDE = 550
SATELLITES_PER_ORBIT = 72
NUMBER_OF_ORBITS = 22
...
def constellationFromSaVi(file_name = TCL_FILE_NAME):
...
#output : rx graph with each vertex corresponding to a satellite and each edge corresponding to the distance between two satellites
...
def graph_full_mesh_from_constellation(constellation):
    ...
    new_graph = rx.generators.path_graph(n_orbits*n_satellites)
    ...
    for orbit1, first_level in enumerate(constellation):
        for satellite1, second_level in enumerate(first_level):
            for orbit2, third_level in enumerate(second_level):
                for satellite2, distance in enumerate(third_level):
                    if distance[1]:
                        new_graph.add_edge(get_node_number(orbit1, satellite1), get_node_number(orbit2, satellite2), weight=distance[0])
    return new_graph
...
#Creates the rx_graph for our constellation
...
#Creates a rx_graph corresponding to our constellation, with edges between all the nodes
